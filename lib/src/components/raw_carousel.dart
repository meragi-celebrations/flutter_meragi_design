...
///
/// The [RawCarouselView] present a scrollable list of items, each of which can dynamically
...
///
/// The [RawCarouselController] is used to control the [RawCarouselController.initialItem].
///
/// The [RawCarouselView.itemExtent] property must be non-null and defines the base
...
///
/// {@tool dartpad}
/// Here is an example of [RawCarouselView] to show the uncontained layout. Each carousel
...
/// See also:
///
///  * [RawCarouselController], which controls the first visible item in the carousel.
///  * [PageView], which is a scrollable list that works page by page.
class RawCarouselView extends StatefulWidget {
  /// Creates a Material Design carousel.
  const RawCarouselView({
    super.key,
    this.padding,
  ...
  /// view is scrolled.
  final RawCarouselController? controller;
...
class _RawCarouselViewState extends State<RawCarouselView> {
  late double _itemExtent;
  RawCarouselController? _internalController;
  RawCarouselController get _controller =>
  ...
  void initState() {
    ...
    if (widget.controller == null) {
      _internalController = RawCarouselController();
    }
  ...
  @override
  void didUpdateWidget(covariant RawCarouselView oldWidget) {
...
/// Scroll physics used by a [RawCarouselView].
///
...
class CarouselScrollPhysics extends ScrollPhysics {
  /// Creates physics for a [RawCarouselView].
  ...
  Simulation? createBallisticSimulation(
    ...
    final CarouselPosition metrics = position as CarouselPosition;
...
/// Metrics for a [RawCarouselView].
class _CarouselMetrics extends FixedScrollMetrics {
  /// Creates an immutable snapshot of values associated with a [RawCarouselView].
  _CarouselMetrics({
...
/// A controller for [RawCarouselView].
///
/// Using a carousel controller helps to show the first visible item on the
/// carousel list.
class RawCarouselController extends ScrollController {
  /// Creates a carousel controller.
  RawCarouselController({
    this.initialItem = 0,
  });

  /// The item that expands to the maximum size when first creating the [RawCarouselView].
  final int initialItem;

  _RawCarouselViewState? _carouselState;

  // ignore: use_setters_to_change_properties
  void _attach(_RawCarouselViewState anchor) {
    _carouselState = anchor;
  }

  void _detach(_RawCarouselViewState anchor) {
  ...
  void attach(ScrollPosition position) {
    ...
    final CarouselPosition carouselPosition = position as CarouselPosition;
