...
///
/// Used by [MDDropdownMenu.filterCallback].
typedef FilterCallback<T> = List<MDDropdownMenuEntry<T>> Function(
    List<MDDropdownMenuEntry<T>> entries, String filter);
...
///
/// Used by [MDDropdownMenu.searchCallback].
typedef SearchCallback<T> = int? Function(
    List<MDDropdownMenuEntry<T>> entries, String query);
...
/// Defines a [MDDropdownMenu] menu button that represents one item view in the menu.
///
/// See also:
///
/// * [MDDropdownMenu]
class MDDropdownMenuEntry<T> {
  /// Creates an entry that is used with [MDDropdownMenu.dropdownMenuEntries].
  const MDDropdownMenuEntry({
    required this.value,
    required this.label,
    this.labelWidget,
    this.leadingIcon,
  ...
  ///
  /// This value must be unique across all entries in a [MDDropdownMenu].
  final T value;
  ...
  ///
  /// The default value is true. If true, the [MDDropdownMenuEntry.label] will be filled
  /// out in the text field of the [MDDropdownMenu] when this entry is clicked; otherwise,
...
///
/// The menu is composed of a list of [MDDropdownMenuEntry]s. People can provide information,
...
///
/// {@tool dartpad}
/// This sample shows how to display outlined [MDDropdownMenu] and filled [MDDropdownMenu].
///
...
///   The [MDDropdownMenu] uses a [TextField] as the "anchor".
/// * [TextField], which is a text input widget that uses an [InputDecoration].
/// * [MDDropdownMenuEntry], which is used to build the [MenuItemButton] in the [MDDropdownMenu] list.
class MDDropdownMenu<T> extends StatefulWidget {
  /// Creates a const [MDDropdownMenu].
  ///
  ...
  /// Determine if the [MDDropdownMenu] is enabled.
  ///
  /// Defaults to true.
  ///
  /// {@tool dartpad}
  ...
  /// Determine the width of the [MDDropdownMenu].
  ///
  /// If this is null, the width of the [MDDropdownMenu] will be the same as the width of the widest
  ...
  /// Text that provides context about the [MDDropdownMenu]'s value, such
  ...
  /// The text style for the [TextField] of the [MDDropdownMenu];
  ///
  /// Defaults to the overall theme's [TextTheme.bodyLarge]
  /// if the dropdown menu theme's value is null.
  final TextStyle? textStyle;

  /// The text align for the [TextField] of the [MDDropdownMenu].
  ///
  ...
  /// Descriptions of the menu items in the [MDDropdownMenu].
  ///
  /// This is a required parameter. It is recommended that at least one [MDDropdownMenuEntry]
  /// is provided. If this is an empty list, the menu will be empty and only
  /// contain space for padding.
  final List<MDDropdownMenuEntry<T>> dropdownMenuEntries;
  ...
  /// by the width of menu items or [MDDropdownMenu.width]. If this property is not null,
  ...
  /// When [MDDropdownMenu.enableFilter] is true, this callback is used to
  ...
  /// [MDDropdownMenu.enableFilter] property is set to true, the default behavior
  ...
  /// When [MDDropdownMenu.enableSearch] is true, this callback is used to compute
  ...
  /// {@end-tool}
  ///
  /// Defaults to null. If this is null and [MDDropdownMenu.enableSearch] is true,
  ...
  @override
  State<MDDropdownMenu<T>> createState() => _MDDropdownMenuState<T>();
}

class _MDDropdownMenuState<T> extends State<MDDropdownMenu<T>> {
  final GlobalKey _anchorKey = GlobalKey();
  final GlobalKey _leadingKey = GlobalKey();
  late List<GlobalKey> buttonItemKeys;
  final MenuController _controller = MenuController();
  late bool _enableFilter;
  late List<MDDropdownMenuEntry<T>> filteredEntries;
  ...
  final List<WidgetStatesController> stateControllerList = [];
  ...
  void initState() {
    ...
    List.generate(filteredEntries.length,
        (i) => stateControllerList.add(WidgetStatesController()));

    _menuHasEnabledItem =
        filteredEntries.any((MDDropdownMenuEntry<T> entry) => entry.enabled);

    final int index = filteredEntries.indexWhere(
        (MDDropdownMenuEntry<T> entry) =>
  ...
  @override
  void didUpdateWidget(MDDropdownMenu<T> oldWidget) {
  ...
  void scrollToHovered({ScrollPositionAlignmentPolicy? alignmentPolicy}) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final BuildContext? hoveredContext =
          buttonItemKeys[currentHovered!].currentContext;
    ...
    }, debugLabel: 'DropdownMenu.scrollToHovered');
  }
  ...
  List<MDDropdownMenuEntry<T>> filter(List<MDDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String filterText = textEditingController.text.toLowerCase();
    return entries
        .where((MDDropdownMenuEntry<T> entry) =>
            entry.label.toLowerCase().contains(filterText))
        .toList();
  }

  int? search(List<MDDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    ...
    final int index = entries.indexWhere((MDDropdownMenuEntry<T> entry) =>
  ...
  List<Widget> _buildButtons(
    List<MDDropdownMenuEntry<T>> filteredEntries,
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      final MDDropdownMenuEntry<T> entry = filteredEntries[i];
      ...
      final Widget menuItemButton = MenuItemButton(
        ...
        statesController: stateControllerList[i],
  ...
  void handleUpKeyInvoke(_) {
    setState(() {
      ...
      stateControllerList.forEach(
        (e) => e.update(WidgetState.hovered, false),
      );

      currentHovered ??= 0;
      currentHovered = (currentHovered! - 1) % filteredEntries.length;
      while (!filteredEntries[currentHovered!].enabled) {
        currentHovered = (currentHovered! - 1) % filteredEntries.length;
      }

      stateControllerList[currentHovered!].update(WidgetState.hovered, true);
  ...
  void handleEnterKeyInvoke(MenuController controller) {
    if (controller.isOpen) {
      MDDropdownMenuEntry<T> entry = filteredEntries[currentHovered!];
  ...
  Widget build(BuildContext context) {
    ...
    final mdInputTheme = InputDecorationThemeData(
      isDense: true,
    ...
    final InputDecorationThemeData effectiveInputDecorationTheme =
        widget.inputDecorationTheme ??
            theme.inputDecorationTheme?.merge(mdInputTheme) ??
            defaults.inputDecorationTheme!.merge(mdInputTheme);
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      onClose: () {
        // Reset current hovered, and remove hovered states from all children
        setState(() {
          currentHovered = null;
        });
        stateControllerList.forEach((e) {
          e.update(WidgetState.hovered, false);
        });
      },
      onOpen: () {
        // Reset the scroll to the first child
        BuildContext? firstContext = buttonItemKeys.firstOrNull?.currentContext;
        if (firstContext != null) {
          Scrollable.ensureVisible(firstContext);
        }
      },
      builder:
        ...
        final Widget textField = TextField(
            ...
            onEditingComplete: () {
              if (currentHighlight != null) {
                final MDDropdownMenuEntry<T> entry =
      ...
      child: Actions(
        actions: <Type, Action<Intent>>{
          ...
          _EnterIntent: CallbackAction<_EnterIntent>(
            onInvoke: (_) {
              handleEnterKeyInvoke(_controller);
            },
          ),
        },
        child: menuAnchor,
