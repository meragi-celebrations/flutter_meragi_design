...
abstract class MouseAreaRegistry {
  /// Register the given [MDMouseAreaRenderBox] with the registry.
  void registerMouseArea(MDMouseAreaRenderBox region);

  /// Unregister the given [MDMouseAreaRenderBox] with the registry.
  void unregisterMouseArea(MDMouseAreaRenderBox region);
...
class MouseAreaSurfaceRenderBox extends RenderProxyBoxWithHitTestBehavior
  ...
  final Set<MDMouseAreaRenderBox> _registeredRegions = <MDMouseAreaRenderBox>{};
  final Map<Object?, Set<MDMouseAreaRenderBox>> _groupIdToRegions =
      <Object?, Set<MDMouseAreaRenderBox>>{};
  ...
  void handleEvent(PointerEvent event, HitTestEntry entry) {
    ...
    final hitRegions = _getRegionsHit(_registeredRegions, result.path)
        .cast<MDMouseAreaRenderBox>()
        .toSet();

    final insideRegions = <MDMouseAreaRenderBox>{
      for (final MDMouseAreaRenderBox region in hitRegions)
  ...
  void registerMouseArea(MDMouseAreaRenderBox region) {
    ...
    if (region.groupId != null) {
      _groupIdToRegions[region.groupId] ??= <MDMouseAreaRenderBox>{};
  ...
  Set<HitTestTarget> _getRegionsHit(
    Set<MDMouseAreaRenderBox> detectors,
...
class MDMouseArea extends SingleChildRenderObjectWidget {
  /// Creates a const [MDMouseArea].
  ///
  ...
  /// Whether or not this [MDMouseArea] is enabled as part of the composite
  /// region.
  final bool enabled;

  /// How to behave during hit testing when deciding how the hit test propagates
  /// to children and whether to consider targets behind this [MDMouseArea].
  ///
  ...
  /// An optional group ID that groups [MDMouseArea]s together so that they
  ...
  void updateRenderObject(
    ...
    covariant MDMouseAreaRenderBox renderObject,
  ) {
    renderObject
      ..registry = MouseAreaRegistry.maybeOf(context)
      ..enabled = enabled
...
class MDMouseAreaRenderBox extends RenderProxyBoxWithHitTestBehavior {
  /// Creates a [MDMouseAreaRenderBox].
  MDMouseAreaRenderBox({
    this.onEnter,
    this.onExit,
  ...
  /// An optional group ID that groups [MDMouseAreaRenderBox]s together so
  ...
  /// The registry that this [MDMouseAreaRenderBox] should register with.
  ///
